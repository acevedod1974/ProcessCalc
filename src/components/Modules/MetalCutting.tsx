...
export function MetalCutting() {
  ...
  const [activeProcess, setActiveProcess] =
    useState<CuttingProcess>("punching");
  const [isCalculating, setIsCalculating] = useState(false);
  const [errors, setErrors] = useState<Record<string, string>>({});

  // Punching parameters
  const [punchingParams, setPunchingParams] = useState<
    Partial<PunchingParameters>
  >({
    material: "",
    thickness: "",
    holeDiameter: "",
  ...
  // Shearing parameters
  const [shearingParams, setShearingParams] = useState<
    Partial<ShearingParameters>
  >({
    material: "",
    thickness: "",
    shearLength: "",
  ...
  // Results
  const [punchingResults, setPunchingResults] =
    useState<PunchingResults | null>(null);
  const [shearingResults, setShearingResults] =
    useState<ShearingResults | null>(null);
  const [clearanceOptimization, setClearanceOptimization] = useState<ReturnType<
    typeof optimizeClearance
  > | null>(null);

  // Material options
  const materialOptions = Object.entries(CUTTING_MATERIALS).map(
    ([key, material]) => ({
      value: key,
      label: material.name,
    })
  ...
  const validatePunchingInputs = (): boolean => {
    ...
    if (
      !punchingParams.holeDiameter ||
      Number(punchingParams.holeDiameter) <= 0
    ) {
      newErrors.holeDiameter = "Hole diameter must be greater than 0";
    }
    if (
      !punchingParams.punchDiameter ||
      Number(punchingParams.punchDiameter) <= 0
    ) {
      newErrors.punchDiameter = "Punch diameter must be greater than 0";
    }
    if (
      Number(punchingParams.punchDiameter) > Number(punchingParams.holeDiameter)
    ) {
      newErrors.punchDiameter =
        "Punch diameter must be less than hole diameter";
    }
    if (
      !punchingParams.clearance ||
      Number(punchingParams.clearance) < 0 ||
      Number(punchingParams.clearance) > 50
    ) {
  ...
  const validateShearingInputs = (): boolean => {
    ...
    if (
      !shearingParams.shearLength ||
      Number(shearingParams.shearLength) <= 0
    ) {
      newErrors.shearLength = "Shear length must be greater than 0";
    }
    if (
      !shearingParams.bladeAngle ||
      Number(shearingParams.bladeAngle) <= 0 ||
      Number(shearingParams.bladeAngle) > 10
    ) {
      newErrors.bladeAngle = "Blade angle must be between 0 and 10 degrees";
    }
    if (
      !shearingParams.clearance ||
      Number(shearingParams.clearance) < 0 ||
      Number(shearingParams.clearance) > 30
    ) {
  ...
  const processes = [
    {
      id: "punching",
      name: "Punching",
      description: "Hole Punching Operations",
      icon: "🎯",
    },
    {
      id: "shearing",
      name: "Shearing",
      description: "Sheet Metal Shearing",
      icon: "✂️",
    },
    {
      id: "blanking",
      name: "Blanking",
      description: "Blanking Operations",
      icon: "📐",
    },
  ];
    ...
    <div className="space-y-6">
            ...
            >
              ...
              <div className="space-y-4">
                <InputField
                  ...
                  onChange={(value) =>
                    setPunchingParams((prev) => ({ ...prev, material: value }))
                  }
                  type="select"
          ...
          {punchingResults && (
            <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  >
                    ...
                    >
                      <p>
                        Actual Clearance:{" "}
                        {punchingResults.clearanceValue.toFixed(3)} mm
                      </p>
                      {clearanceOptimization && (
                        <>
                          <p>
                            Optimal Clearance:{" "}
                            {clearanceOptimization.optimal.toFixed(3)} mm
                          </p>
                          <p>
                            Optimal Percentage:{" "}
                            {clearanceOptimization.percentage.toFixed(1)}%
                          </p>
                        </>
                      )}
                    </div>
              ...
              <RecommendationCard
                ...
                type={
                  punchingResults.cutQuality === "Excellent"
                    ? "success"
                    : punchingResults.cutQuality === "Poor"
                    ? "warning"
                    : "info"
                }
              />
            ...
            >
              ...
              <div className="space-y-4">
                <InputField
                  ...
                  onChange={(value) =>
                    setShearingParams((prev) => ({ ...prev, material: value }))
                  }
                  type="select"
                  options={materialOptions}
                  placeholder="Select Material..."
      ...
      >
        <div className="flex items-start space-x-3">
          <AlertCircle
            className={`${isDark ? "text-orange-300" : "text-orange-700"} mt-1`}
            size={20}
          />
          <div>
            <h4
            ...
            >
              <p>
                • Calculations include material properties, temperature effects,
                and tool wear considerations
              </p>
              <p>
                • Clearance optimization is based on material hardness and
                thickness relationships
              </p>
              <p>
                • Tool life predictions are estimates based on standard wear
                models
              </p>
              <p>
                • Cut quality assessment considers clearance ratio and process
                parameters
              </p>
              {activeProcess === "punching" && (
                <p>
                  • Punching force includes both shearing and stripping
                  components
                </p>
              )}
              {activeProcess === "shearing" && (
                <p>
                  • Shearing calculations account for blade angle and hold-down
                  pressure effects
                </p>
              )}
            </div>
